...
echo "[wrapper] FILEID    = " ${FILEID} #outname
echo "[wrapper] FILE      = " ${FILE}   #inname
...
echo "
{
gROOT->ProcessLine(\".L processNtuple.C\");
processNtuple(\"${FILEID}\", \"${FILE}\");
}
" > runme.C
...
lcg-cp -b -D srmv2 --vo cms -t 2400 --verbose file:`pwd`/output/${OUTPUT} srm://bsrm-3.t2.ucsd.edu:8443/srm/v2/server?SFN=${COPYDIR}/${OUTPUT}
